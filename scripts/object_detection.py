...
def parse_args() -> argparse.Namespace:
    ...
    # Validate network and labels file exist
    if not os.path.exists(args.net):
        raise FileNotFoundError(f"Network file not found: {args.net}")
    if not os.path.exists(args.labels):
        raise FileNotFoundError(f"Labels file not found: {args.labels}")

    # If input is not camera, validate input path exists
    if args.input.lower() != "camera" and not os.path.exists(args.input):
...
def init_picamera_stream(resolution):
    picam2 = Picamera2()
...
def infer(net, input_source, batch_size, labels, output_dir,
    ...
    if input_source.lower() == "camera":
        images = init_picamera_stream(resolution)
        cap = None  # No cv2.VideoCapture for PiCamera
    else:
